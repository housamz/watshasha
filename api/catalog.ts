...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (config.type === "movie") {
      const response = await tmdb.getArabicMovies(
        tmdbPage,
        config.language,
        config.sort,
      );
      const items = response.results.filter((item) => isArabicContent(item));
      const imdbIds = await Promise.allSettled(
        items.map((item) => tmdb.getImdbId("movie", item.id)),
      );
      ...
      const response = await tmdb.getArabicTvSeries(
        tmdbPage,
        config.language,
        config.sort,
      );
      const items = response.results.filter((item) => isArabicContent(item));
      const imdbIds = await Promise.allSettled(
        items.map((item) => tmdb.getImdbId("series", item.id)),
      );
      metas = items.map((item, index) =>
        mapTvToCatalogItem(
          item,
