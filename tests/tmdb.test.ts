...
describe("TmdbService", () => {
  ...
  it("returns empty results without credentials", async () => {
    ...
    await expect(service.getArabicMovies()).resolves.toMatchObject({
      results: [],
    });
  ...
  it("rejects malformed upstream responses", async () => {
    ...
    const service = new TmdbService({ apiKey: "test-key", fetchFn });
    await expect(service.getArabicMovies()).rejects.toThrow(
  ...
  it("resolves IMDb IDs through TMDB external IDs", async () => {
    ...
    expect(String(vi.mocked(fetchFn).mock.calls[0][0])).toContain(
      "/movie/123/external_ids",
    );
  });
  ...
  it("finds a TMDB series from an IMDb ID", async () => {
    ...
    const service = new TmdbService({ apiKey: "test-key", fetchFn });
    await expect(
      service.findTmdbIdByImdbId("series", "tt7654321"),
    ).resolves.toBe(456);
  });
});
