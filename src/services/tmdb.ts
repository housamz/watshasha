...
export class TmdbService {
  ...
  public async getArabicMovies(
    ...
    const result = await this.request<TmdbDiscoverResponse<TmdbMovieItem>>(
      "/discover/movie",
      {
  ...
  public async getArabicTvSeries(
    ...
    const result = await this.request<TmdbDiscoverResponse<TmdbTvItem>>(
      "/discover/tv",
      {
        with_original_language: language,
        sort_by: sort,
  ...
  public async getImdbId(
    ...
    const result = await this.request<TmdbExternalIds>(
      `/${resource}/${tmdbId}/external_ids`,
    );
    const imdbId = result?.imdb_id;
    return imdbId && isImdbId(imdbId) ? imdbId : null;
  }

  public async findTmdbIdByImdbId(
    type: MediaType,
    imdbId: string,
  ): Promise<number | null> {
    if (!this.apiKey && !this.accessToken) return null;
    ...
    const match =
      type === "movie" ? result?.movie_results?.[0] : result?.tv_results?.[0];
