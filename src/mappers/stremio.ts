...
export function getImageUrl(
  ...
  const base =
    size === "poster" ? TMDB_IMAGE_BASE_POSTER : TMDB_IMAGE_BASE_BACKDROP;
...
export function mapMovieToCatalogItem(
  movie: TmdbMovieItem,
  imdbId?: string | null,
): StremioCatalogItem {
...
export function mapTvToCatalogItem(
  tv: TmdbTvItem,
  imdbId?: string | null,
): StremioCatalogItem {
...
export function mapMovieDetailsToMeta(
  movie: TmdbMovieDetails,
  preferredId?: string,
): StremioMetaDetail {
  const catalogItem = mapMovieToCatalogItem(
    movie,
    preferredId || movie.imdb_id,
  );
  const year = movie.release_date
    ? movie.release_date.split("-")[0]
    : undefined;
...
export function mapTvEpisodesToVideos(
  imdbId: string,
  ...
  return seasons
    ...
    .map((episode) => ({
      ...
      thumbnail: episode.still_path
        ? `${TMDB_IMAGE_BASE_STILL}${episode.still_path}`
        : undefined,
    }));
}
...
export function mapTvDetailsToMeta(
  ...
  const episodeImdbId =
    ...
    (preferredId && isImdbId(preferredId) ? preferredId : undefined);
  ...
  const cast = tv.credits?.cast
    ...
    : undefined;
  const creators = tv.created_by ? tv.created_by.map((c) => c.name) : undefined;
  const country =
  ...
  return {
    ...
    genres: genres && genres.length > 0 ? genres : undefined,
    videos:
      episodeImdbId && seasons.length > 0
        ? mapTvEpisodesToVideos(episodeImdbId, seasons)
        : undefined,
  };
}
